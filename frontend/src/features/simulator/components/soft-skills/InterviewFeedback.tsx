...
const InterviewFeedback: React.FC<InterviewFeedbackProps> = ({ feedback }) => {
  const metricLabels = [
    ...
    { key: 'clarity', label: 'Estructuración de Ideas / Claridad', color: 'bg-amber-500' },
    { key: 'confidence', label: 'Nivel de Confianza percibido', color: 'bg-cyan-500' },
  ];

  return (
    <div className="bg-white border border-base-200 rounded-2xl p-5 shadow-md animate-fadeIn">
      <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
        <span className="p-1.5 bg-blue-50 rounded-lg text-blue-600 flex items-center justify-center">
          ...
          return (
            <div key={metric.key}>
              <div className="flex justify-between text-xs text-slate-600 font-bold mb-1">
                ...
                <span className="font-mono text-slate-900 font-black">{value} / 100</span>
              </div>
      ...
      <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/50">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Feedback Detallado</h4>
        <p className="text-sm text-slate-700 leading-relaxed font-medium">{feedback.constructiveFeedback}</p>
      </div>
    </div>
  );
};
